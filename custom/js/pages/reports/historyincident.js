...
async function showIncidentsData() {
  ...
  if(fromDate == '') return AlertService.showAlertError('No datetime', "Please choose datetime!", 6000);
  if(toDate == '') return AlertService.showAlertError('No datetime', "Please choose datetime!", 6000);
...
async function showIncidentListDefault(){
  ...
  fromDate = TimeService.changeFormatDateTime(fromDate);
  toDate = TimeService.changeFormatDateTime(toDate);
  let sentData = { fromDate, toDate, IncidentID: 0 };
  let data = await IncidentService.getIncidentsData(sentData);
  arrIncidents.length = 0;
  if(data) {
    showIncidentListPagination(data);
...
function renderIncidentsTable(data) {
  ...
  if(data){
    data.forEach(incident => {
      const { sGuardName, sZoneName, iTimeSpent, dDateTimeIntinial, dDateTimeStart, dDateTimeEnd, sAlertDescription, ImageUrl, sRouteName, ProcessAlert } = incident;
